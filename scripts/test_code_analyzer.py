...
async def test_dff_only():
    """Script to test the CodeAnalyzer with specific DFF data."""
    print("Starting CodeAnalyzer DFF test with specific data...")
    
    # Create CodeAnalyzer instance
    code_analyzer = CodeAnalyzer()
    print("✅ CodeAnalyzer instance created")
    ...
    try:
        ...
        print("\nAnalyzing DFF data...")
        dff_result = await code_analyzer.execute(dff_data=dff_data)
        ...
        if 'results' in dff_result:
            for file_result in dff_result['results']:
                ...
                if 'details' in file_result:
                    print("\nDetails:")
                    if 'code_structure' in file_result['details']:
                        structure = file_result['details']['code_structure']
                        print(f"- Functions: {structure.get('functions', 'N/A')}")
                        print(f"- Classes: {structure.get('classes', 'N/A')}")
                        print(f"- Complexity: {structure.get('complexity', 'N/A')}")
                    
                    print(f"- Maintainability: {file_result['details'].get('maintainability', 'N/A')}/10")
                    print(f"- Readability: {file_result['details'].get('readability', 'N/A')}/10")
                    print(f"- Modularity: {file_result['details'].get('modularity', 'N/A')}/10")
                    
                    # Display patterns if available
                    if 'patterns' in file_result['details']:
                        print("\nPatterns:")
                        for pattern in file_result['details']['patterns']:
                            print(f"- {pattern.get('name', 'Unknown')}: {pattern.get('description', 'No description')}")
